...
void pick_month(int iteration, std::ofstream& os);
//prec: requires os to be accessing a file,
//postc: same as above, except outputs to the file being accessed by os
...
void output_table(const	double difference[], const double actuals[]);
...
void output_table(const double difference[], const double actuals[], std::string filename);
//same as above excepts opens filename through an ofstream object
//and outputs to that file, then closes stream

void output_graph(const double averages[], const double actuals[]);
...
void output_graph(const double averages[], const double actuals[], std::string filename);
//same as above excepts opens filename through an ofstream object
//and outputs to that file, then closes stream
...
int main()
{
	...
	do
	{
		...
		else if (choice == '1')
		{
			...
			cout << "If you would like to write this table on to a file,\n"
				<< "type 1, then press enter; otherwise, type 0, then\n"
				<< "press enter:\n";
			cin >> choice;
			{
				if (choice == '1')
				{
					...
					cin >> filename;
					output_table(difference, actual, filename);
					cout << "Done.\n";
				}
			}
		}
		...
		{
			output_graph(average, actual);

			cout << "If you would like to write this graph on to a file,\n"
				<< "type 1, then press enter; otherwise, type 0, then\n"
				<< "press enter:\n";
			cin >> choice;
			{
				if (choice == '1')
				{
					...
					cout << "Enter the file name:\n";
					cin >> filename;
					output_graph(average, actual, filename);
					cout << "Done.\n";
				}
			}
		}
...
//output_table
void output_table(const double difference[],const double actual[])
{
	using namespace std;
...
void output_table(const double difference[], const double actual[], std::string filename)
{
	using namespace std;
	...
	if (os.fail())
	{
		...
		cout << "Failed to open output file.\n"
			<< "Enter another file name to try again,\n"
...
void output_graph(const double average[], const double actual[])
{
	using namespace std;
...
void output_graph(const double average[], const double actual[], std::string filename)
{
	using namespace std;
	...
	{
		os << endl;
		pick_month(i, os);//outputs name of month
		os << ":\n";

		os << "Average: ";
		for (int index = 0; index < average[i] * 25 / max; index++)
			os << "*"; //writes "average[i] * 25 / max" asterisks
		os << endl;	//if the ith average is max, outputs 25 asterisks
